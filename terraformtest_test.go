...
func TestTFAWS101NatEIPOne(t *testing.T) {
	...
	want := terraformtest.TFResource{
		Filter: `planned_values.root_module.child_modules.#.resources.0`,
		Check: map[string]string{
			"0.address":               "module.vpc.aws_eip.nat[0]",
			"0.type":                  "aws_eip",
			"0.values.vpc":            "true",
			"0.values.tags.Terraform": "true",
			"0.values.timeouts":       "",
		},
	}

	got, err := terraformtest.ReadTfPlan("terraform-aws-101.tfplan.json")
	if err != nil {
		t.Fatalf("cannot read terraform plan: %v", err)
	}
...
func TestTFAWS101DBOptionGroup(t *testing.T) {
	...
	want := terraformtest.TFResource{
		...
		Check: map[string]string{
			...
			"option.option_name":   "MARIADB_AUDIT_PLUGIN",
		},
	}
